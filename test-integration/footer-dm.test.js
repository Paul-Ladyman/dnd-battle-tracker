...
describe('DM Tips', () => {
  test('the dm tips button is not expanded by default', async () => {
    render(<DungeonMasterAppWrapper />);
    const button = await screen.findByRole('button', { name: 'Dungeon Master tips' });
    expect(button).toHaveAttribute('aria-expanded', 'false');
  });

  test('the dm tips are hidden by default', async () => {
    render(<DungeonMasterAppWrapper />);
    const tips = await screen.findByTestId('dm-tips');
    expect(tips).toBeInTheDocument();
    expect(tips).toHaveStyle({ display: 'none' });
  });

  test('the dm tips button is expanded when clicked', async () => {
    render(<DungeonMasterAppWrapper />);
    const user = userEvent.setup();
    const button = await screen.findByRole('button', { name: 'Dungeon Master tips' });
    await user.click(button);
  ...
  test('the dm tips are displayed as a list', async () => {
    render(<DungeonMasterAppWrapper />);
    const user = userEvent.setup();
    const button = await screen.findByRole('button', { name: 'Dungeon Master tips' });
    await user.click(button);
    const tips = await screen.findByTestId('dm-tips');
    const list = await findByRole(tips, 'list');
    expect(list).toBeInTheDocument();
    expect(list.childNodes.length).toBeGreaterThan(0);
  });

  test('the dm tips are displayed when the button is clicked', async () => {
    render(<DungeonMasterAppWrapper />);
    const user = userEvent.setup();
    const button = await screen.findByRole('button', { name: 'Dungeon Master tips' });
    await user.click(button);
    const tips = await screen.findByTestId('dm-tips');
    expect(tips).toBeInTheDocument();
    expect(tips).toHaveStyle({ display: 'block' });
  });

  test('the dm tips button is not expanded when clicked again', async () => {
    render(<DungeonMasterAppWrapper />);
    const user = userEvent.setup();
    const button = await screen.findByRole('button', { name: 'Dungeon Master tips' });
    await user.click(button);
  ...
  test('the dm tips are hidden when the button is clicked', async () => {
    render(<DungeonMasterAppWrapper />);
    const user = userEvent.setup();
    const button = await screen.findByRole('button', { name: 'Dungeon Master tips' });
    await user.click(button);
